            ...
            <div className="space-y-6 max-w-7xl mx-auto">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h1 className="text-2xl font-bold tracking-tight text-white mb-0.5">Good day, Manager! 👋</h1>
                                    <p className="text-zinc-400 text-sm font-medium">Project Overview & Approvals</p>
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <Card className="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm p-6 rounded-3xl">
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    ...
                                    <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Volume of material requests over the last 6 months.</p>
                                </div>
                            </div>

                            <div className="h-[280px] w-full mt-4">
                                {stats?.materialRequestsOverTime?.length > 0 ? (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={stats.materialRequestsOverTime} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                            <defs>
                        ...
                        <Card className="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm p-6 rounded-3xl">
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    ...
                                    <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Financial overview for active projects.</p>
                                </div>
                                <span className="px-3 py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 text-xs font-semibold rounded-full">This Quarter</span>
                            </div>
                    ...
                    <div className="space-y-6">
                        <Card className="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm p-6 rounded-3xl h-full">
    ...
    return (
        <div className="group">
            <div className="flex justify-between mb-3 items-end">
                <div>
                    <p className="text-base font-bold text-zinc-800 dark:text-zinc-200 leading-tight">{name}</p>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium mt-1">Budget: ₱{budget}</p>
                </div>
                <span className="text-sm font-black text-zinc-900 dark:text-white tracking-tight">{progress}%</span>
            </div>
            <div className="h-2.5 bg-zinc-100 dark:bg-zinc-800/50 rounded-full overflow-hidden border border-zinc-200/50 dark:border-zinc-800">
                <div
...
function ModernQuickAction({ href, icon, label, brand }) {
    ...
    return (
        <a
            href={href}
            className={`group flex flex-col items-center justify-center gap-3 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800/50 bg-zinc-50/50 dark:bg-zinc-800/30 transition-all duration-300 hover:shadow-md ${brands[brand]}`}
        >
            <div className="p-3 bg-white dark:bg-zinc-900 rounded-xl shadow-sm group-hover:scale-110 transition-transform duration-300">
                {icon}
            </div>
            <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300 transition-colors duration-200 text-center leading-tight">
                {label}
            </span>
        </a>
    );
